...
@section('main')
    <div class="max-w-6xl mx-auto p-4">
        <div class="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-100">
            ...
            <div class="p-6">
                ...
                <form action="{{ route('userpengajuancutiumum.store') }}" method="POST" enctype="multipart/form-data"
                    id="formCuti">
                    @csrf
                    ...
                    <div class="mb-6">
                        ...
                        <input type="hidden" name="jeniscuti_id" id="jeniscuti_id" value="{{ old('jeniscuti_id') }}">

                        {{-- Tab pills --}}
                        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2" id="jeniscuti-tabs">
                            @foreach ($jenisCuti as $cuti)
                                <button type="button"
                                    class="jenis-cuti-tab w-full px-3 py-2 rounded-full border text-sm font-medium text-center transition-all duration-200
                                        {{ old('jeniscuti_id') == $cuti->id
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <div>
                                    ...
                                    <div class="flex items-center gap-3 w-full px-4 py-2.5 bg-green-50 border border-green-200 rounded-lg h-[46px]">
                                        <div class="p-1.5 rounded-full bg-green-100 text-green-600 flex-shrink-0">
                                            <i class="fas fa-briefcase text-xs"></i>
                                        </div>
                                        <div class="flex items-center gap-2">
                                            <p class="text-green-700 font-bold text-sm">{{ $masakerja->jumlah_masa_kerja }}</p>
                                        </div>
                                    </div>
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Masa Kerja</label>
                                    <div class="flex items-center gap-2 w-full px-4 py-2.5 bg-orange-50 border border-orange-200 rounded-lg h-[46px]">
                                        <div class="p-1.5 rounded-full bg-orange-100 text-orange-500 flex-shrink-0">
                                            <i class="fas fa-exclamation-triangle text-xs"></i>
                                        </div>
                                        <p class="text-orange-600 text-xs flex-1 leading-tight">Belum dihitung</p>
                                        <a href="{{ route('usermasakerja.create') }}"
                                        class="inline-flex items-center gap-1.5 px-3 py-1 bg-white border border-orange-300 text-orange-600 rounded-md hover:bg-orange-50 transition-colors text-xs font-medium flex-shrink-0">
                                            <i class="fas fa-calculator"></i>
                                            Hitung
                                        </a>
                                    </div>
                                    <input type="hidden" name="masa_kerja" value="">
                                </div>
                                @endif
                        </div>
    ...
    <style>
        ...
        .jenis-cuti-tab.active {
            background-color: var(--color-primary, #65a30d);
            color: #ffffff;
        ...
        $(document).ready(function() {
            ...
            function toggleLampiranVisibility() {
                ...
                if (jenisCutiId && lampiranContainer) {
                    const selectedOptionNama = $('#jeniscuti_id').data('nama') || '';
                    if (selectedOptionNama.includes('Sakit') || selectedOptionNama.includes('Alasan Penting')) {
            ...
            function setupEventHandlers() {
                ...
                // 1. EVENT KLIK UNTUK TOMBOL TAB JENIS CUTI
                $('.jenis-cuti-tab').on('click', function() {
                    // Ubah warna tombol yang aktif
                    $('.jenis-cuti-tab')
                        .removeClass('bg-primary text-white border-primary shadow-sm')
                        .addClass('bg-white text-gray-600 border-gray-300');
                    
                    $(this)
                        .removeClass('bg-white text-gray-600 border-gray-300')
                        .addClass('bg-primary text-white border-primary shadow-sm');

                    // Ambil data dari tombol yang diklik
                    const id = $(this).data('id');
                    const nama = $(this).data('nama');

                    // Masukkan ke hidden input dan simpan namanya
                    $('#jeniscuti_id').val(id);
                    $('#jeniscuti_id').data('nama', nama);

                    // Pancing event change agar form di bawahnya muncul
                    $('#jeniscuti_id').trigger('change');
                });

                // 2. LOGIKA KETIKA JENIS CUTI BERUBAH
                $('#jeniscuti_id').on('change', function() {
                    const jenisCutiId = $(this).val();
                    // Sekarang kita ambil namanya dari data atribut hidden input
                    ...
                    if (jenisCutiId) {
                        ...
                        if (jenisCutiNama === 'Cuti Sakit' || jenisCutiNama === 'Cuti Alasan Penting') {
                            ...
                            $('#tgl_selesai').prop('readonly', true).removeClass('bg-white').addClass(
                                'bg-gray-100');
                            $('.durasi-btn').removeClass('bg-primary/30 text-white').addClass(
                                'text-blue-700 bg-white');
                            if ($('#tgl_mulai').val()) updateCutiBesar($('.durasi-btn.bg-primary\\/30').data(
                                'durasi') || 3);
                ...
                // 3. TRIGGER SAAT TANGGAL BERUBAH
                $('#tgl_mulai, #tgl_selesai').on('change', function() {
                    const jenisCutiNama = $('#jeniscuti_id').data('nama'); // Ambil dari data atribut

                    if (jenisCutiNama === 'Cuti Melahirkan') {
                        if ($('#tgl_mulai').val()) hitungCutiMelahirkan();
                    } else if (jenisCutiNama === 'Cuti Besar') {
                        if ($('#tgl_mulai').val()) updateCutiBesar($('.durasi-btn.bg-primary\\/30').data('durasi') || 3);
                ...
                $('.durasi-btn').on('click', function() {
                    ...
                    $('.durasi-btn').removeClass('bg-primary/30 border-primary').addClass('bg-primary/10');
                    $(this).removeClass('bg-primary/10').addClass('bg-primary/30 border-primary');
