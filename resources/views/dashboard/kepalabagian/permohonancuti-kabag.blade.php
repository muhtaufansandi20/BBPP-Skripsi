...
@section('content')
<div class="p-6 bg-gradient-to-br from-gray-50 to-orange-50 rounded-2xl shadow-xl">
    <!-- Header Section -->
    <div class="mb-6">
        <div class="flex items-center gap-4 mb-2">
            <h2 class="text-xl sm:text-2xl font-bold text-gray-800">Daftar Pengajuan Cuti Tahunan</h2>
            <span class="px-3 py-1 text-xs font-medium rounded-full bg-gradient-to-r from-emerald-100 to-green-100 text-emerald-800 border border-emerald-200">
                Kepala Bagian
            </span>
        </div>
        
        <!-- Info Card -->
        <div class="mb-6 p-4 rounded-xl bg-gradient-to-r from-emerald-50 to-green-50 border border-emerald-200 shadow-sm">
            <div class="flex items-start gap-3">
                <div class="flex-shrink-0 p-2 rounded-lg bg-green-100 text-green-600">
                ...
                <div>
                    <p class="text-sm text-orange-700">
                        Halaman ini untuk mengelola pengajuan cuti tahunan pegawai. Anda dapat menyetujui atau menolak pengajuan cuti sesuai dengan kebijakan yang berlaku.
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="overflow-x-auto max-h-[600px]">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @php
                        $pendingApplications = $ctStatusAdminKatimker->whereNull('ctStatusKatimkerKabag');
                    @endphp
                        ...
                        @foreach ($pendingApplications as $index => $cuti)
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{ \Carbon\Carbon::parse($cuti->statusUserAdmin->pengajuanCutiTahunan->tgl_pengajuan)->translatedFormat('d M Y') }}
                                </td>
                                <td class="px-6 py-4">
                                    <div class="text-sm font-medium text-gray-900">{{ $cuti->statusUserAdmin->pengajuanCutiTahunan->user->name }}</div>
                                    <div class="text-xs text-gray-500">{{ $cuti->statusUserAdmin->pengajuanCutiTahunan->user->nip ?? '-' }}</div>
                                    <div class="text-xs text-gray-400">{{ $cuti->statusUserAdmin->pengajuanCutiTahunan->user->jabatan ?? '-' }}</div>
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-500">
                                    <div class="space-y-1">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span>Mulai: {{ \Carbon\Carbon::parse($cuti->statusUserAdmin->pengajuanCutiTahunan->tgl_mulai)->format('d/m/Y') }}</span>
                                        </div>
                                        <div class="flex items-center gap-2">
                                            <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                                            </svg>
                                            <span>Selesai: {{ \Carbon\Carbon::parse($cuti->statusUserAdmin->pengajuanCutiTahunan->tgl_selesai)->format('d/m/Y') }}</span>
                                        </div>
                                        <div class="flex items-center gap-2">
                                            <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                            </svg>
                                            <span>Lama: {{ $cuti->statusUserAdmin->pengajuanCutiTahunan->lama_cuti }} Hari</span>
                                        </div>
                                        <div class="flex items-start gap-2 mt-2">
                                            ...
                                            <span class="text-gray-600 italic">Alasan: {{ $cuti->statusUserAdmin->pengajuanCutiTahunan->alasan }}</span>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <a href="{{ route('kabagdatapermohonancuti.show', $cuti->id) }}" 
    ...
    <div id="completed-table" class="bg-white rounded-xl shadow-md overflow-hidden hidden">
        ...
        <div class="overflow-x-auto max-h-[600px]">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @php
                        $verifiedApplications = $ctStatusAdminKatimker->whereNotNull('ctStatusKatimkerKabag');
                    @endphp
                    ...
                    @if($verifiedApplications->count() > 0)
                        @foreach ($verifiedApplications as $index => $cuti)
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    @php
                                        $statusText = $cuti->ctStatusKatimkerKabag->status;
                                        $bgClass = 'bg-gray-100';
