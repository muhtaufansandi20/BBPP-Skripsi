...
@section('content')
    <div class="px-6 pt-0 pb-6 bg-gradient-to-br from-gray-50 to-indigo-50 rounded-2xl shadow-xl">
        @if (!$hasWorkPeriod)
            ...
            <div class="text-center p-8 rounded-xl bg-gradient-to-r from-orange-50 to-amber-50 border border-amber-200 shadow-sm">
                <div class="mx-auto w-16 h-16 rounded-full bg-amber-100 text-amber-600 flex items-center justify-center mb-4">
                    <i class="fas fa-calendar-times text-2xl"></i>
                </div>
                <h3 class="text-xl font-bold text-amber-700 mb-2">Masa Kerja Belum Diisi</h3>
                <p class="text-amber-600 mb-4">Silakan lengkapi data masa kerja Anda sebelum mengajukan cuti.</p>
        ...
        @elseif (!$checkVerifikasi)
            ...
            <div class="text-center p-8 rounded-xl bg-gradient-to-r from-orange-50 to-amber-50 border border-amber-200 shadow-sm">
                ...
                <h3 class="text-xl font-bold text-amber-700 mb-2">Kuota Cuti Belum Diverifikasi</h3>
                <p class="text-amber-600 mb-4">Silakan hubungi administrator untuk memverifikasi kuota cuti tahunan Anda.</p>
                ...
                @if (isset($admin) && !empty($admin->no_hp))
                    ...
                    <a href="https://wa.me/{{ $whatsappNumber }}?text={{ $quotaMessage }}" target="_blank"
                        class="inline-flex items-center px-5 py-2 bg-gradient-to-r from-green-500 to-emerald-500 text-white rounded-lg hover:from-green-600 hover:to-emerald-600 transition">
        ...
        @else
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-4 pt-4">
                ...
                <a href="{{ route('kabagpengajuancutitahunan.create') }}"
                    ...
                    <table class="w-full table-auto bg-white rounded-xl overflow-hidden">
                        <thead>
                            <tr class="bg-gradient-to-r from-primary to-lime-500 text-white">
                                <th class="py-4 px-4 text-center font-semibold uppercase tracking-wider text-sm">NO</th>
                                <th class="py-4 px-4 text-center font-semibold uppercase tracking-wider text-sm">TANGGAL PENGAJUAN</th>
                                <th class="py-4 px-4 text-center font-semibold uppercase tracking-wider text-sm">PERIODE CUTI</th>
                                <th class="py-4 px-4 text-center font-semibold uppercase tracking-wider text-sm">LAMA CUTI</th>
                                <th class="py-4 px-4 text-center font-semibold uppercase tracking-wider text-sm">STATUS</th>
                                <th class="py-4 px-4 text-center font-semibold uppercase tracking-wider text-sm">CATATAN</th>
                                <th class="py-4 px-4 text-center font-semibold uppercase tracking-wider text-sm">AKSI</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100 text-sm">
                            @foreach ($pengajuancutitahunan as $index => $cuti)
                                @php
                                    ...
                                    $isCanceled = isset($cuti->statusCutiTahunan) && $cuti->statusCutiTahunan->status == 'dibatalkan';
                                @endphp
                                <tr class="hover:bg-indigo-50 transition-colors duration-200 group text-center"
                                    ...
                                    <td class="py-4 px-2 whitespace-nowrap text-gray-700">
                                        <div class="flex flex-col items-center gap-1">
                                            <span class="flex items-center gap-1.5">
                                                <i class="far fa-play-circle text-emerald-500 text-xs"></i>
                                    ...
                                    <td class="py-4 px-2 whitespace-nowrap">
                                        <span class="px-3 py-1.5 rounded-full bg-emerald-100 text-emerald-700 font-semibold text-xs inline-flex items-center gap-1.5">
                                    ...
                                    <td class="py-4 px-2 whitespace-nowrap">
                                        @php
                                            ...
                                            $icon = 'fas fa-circle text-[10px] text-yellow-500';
                                    ...
                                    <td class="py-4 px-2 text-gray-700 max-w-xs text-center">
                                        @php
                                            ...
                                            $catatanKabal = optional(optional($cuti->ctStatusUserAdmin)->ctStatusAdminKabal)->catatan ?? null;

                                            $catatan = [];
                                            if ($catatanAdmin) $catatan[] = "<div class='mb-1.5 text-left'><span class='font-semibold text-blue-600 text-xs'>Admin:</span><p class='text-xs text-gray-600 mt-0.5 pl-3'>{$catatanAdmin}</p></div>";
                                            if ($catatanKabal) $catatan[] = "<div class='mb-1.5 text-left'><span class='font-semibold text-green-600 text-xs'>Kepala Balai:</span><p class='text-xs text-gray-600 mt-0.5 pl-3'>{$catatanKabal}</p></div>";
                                        @endphp
                                        ...
                                        @else
                                            <span class="text-gray-500 text-xs">Tidak ada catatan</span>
                                        @endif
                                    </td>
                                    ...
                                    <td class="py-4 px-2 whitespace-nowrap">
                                        <div class="flex items-center justify-center gap-1.5">
                                            {{-- Tombol Detail --}}
                                            <a href="{{ route('kabagpengajuancutitahunan.show', $cuti->id) }}"
                                                class="inline-flex items-center justify-center w-8 h-8 rounded-md bg-emerald-50 text-emerald-600 hover:bg-emerald-600 hover:text-white transition-colors duration-150 border border-emerald-200"
                                                title="Detail">
                                            ...
                                            @if (!$isCanceled)
                                                <button type="button"
                                                    onclick="openCancelModal('{{ route('kabagpengajuancutitahunan.cancel', $cuti->id) }}', '{{ $cuti->id }}')"
