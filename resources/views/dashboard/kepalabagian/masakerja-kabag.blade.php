...
<div id="masaKerjaModal" class="fixed inset-0 bg-gray-900 bg-opacity-50 hidden flex items-center justify-center z-50">
    <div class="max-w-lg w-full mx-auto bg-white p-6 rounded-lg shadow-lg transform transition-all">
        ...
        <!-- Critical Fix: Form handling for update/create with updated route names -->
        @if($masakerja)
            <form action="{{ route('kabagmasakerja.update', $masakerja->id) }}" method="POST" class="mt-4" id="masaKerjaForm">
                @csrf
                @method('PUT')
                <input type="hidden" name="user_id" value="{{ auth()->user()->id }}">
                <input type="hidden" name="jumlah_masa_kerja" id="masaKerjaInput">
        @else
            <form action="{{ route('kabagmasakerja.store') }}" method="POST" class="mt-4" id="masaKerjaForm">
                @csrf
                <input type="hidden" name="user_id" value="{{ auth()->user()->id }}">
                <input type="hidden" name="jumlah_masa_kerja" id="masaKerjaInput">
        @endif
