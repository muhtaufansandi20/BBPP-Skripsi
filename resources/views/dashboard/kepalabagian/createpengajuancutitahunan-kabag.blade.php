...
<div class="max-w-3xl mx-auto bg-white px-6 py-8 pt-6 rounded-xl shadow-lg border border-gray-100">
    ...
    <!-- Form Section -->
    <form action="{{ route('kabagpengajuancutitahunan.store') }}" method="POST" class="space-y-6" id="leaveForm">
        @csrf
...
<script>
    ...
    document.getElementById('tanggalCutiMulai').addEventListener('change', function() {
        validateDates();
    });

    document.getElementById('tanggalCutiSelesai').addEventListener('change', function() {
        validateDates();
    });

    function validateDates() {
        const startDate = new Date(document.getElementById('tanggalCutiMulai').value);
        const endDate = new Date(document.getElementById('tanggalCutiSelesai').value);
        const errorElement = document.getElementById('dateError');

        if (startDate && endDate && endDate < startDate) {
            errorElement.classList.remove('hidden');
            document.getElementById('lama_cuti').value = '0';
        } else {
            errorElement.classList.add('hidden');
            // Add logic to calculate working days if needed
        }
    }

    // Phone number validation
    document.getElementById('no_hp_cuti').addEventListener('input', function() {
