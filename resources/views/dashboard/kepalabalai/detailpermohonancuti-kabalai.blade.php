...
<div class="max-w-6xl mx-auto p-6 pt-0">
    ...
    <div class="flex items-center gap-4 mb-6 group">
        ...
        <div class="w-full">
            <h2 class="text-xl sm:text-2xl font-bold text-gray-800">Detail Pengajuan Cuti</h2>
        ...
        <div class="p-6 space-y-0 divide-y divide-gray-100">
            <div class="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-gray-100">
                <div class="py-4 px-2">
                    ...
                    <p class="text-sm font-medium text-gray-800">{{ $ctstatuskatimkerkabag->name ?? '-' }}</p>
                </div>
                <div class="py-4 px-2 md:pl-6">
                    ...
                    <p class="text-sm font-medium text-gray-800">{{ $ctstatuskatimkerkabag->nip ?? '-' }}</p>
                </div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-gray-100">
                <div class="py-4 px-2">
                    ...
                    <p class="text-sm font-medium text-gray-800">{{ $ctstatuskatimkerkabag->jabatan ?? '-' }}</p>
                </div>
                <div class="py-4 px-2 md:pl-6">
                    <p class="text-xs font-medium text-gray-400 uppercase tracking-widest mb-1.5 flex items-center gap-1.5">
                    ...
                    <p class="text-sm font-medium text-gray-800">{{ $ctstatuskatimkerkabag->masa_kerja ?? '-' }}</p>
                </div>
            </div>
            <div class="py-4 px-2">
                <p class="text-xs font-medium text-gray-400 uppercase tracking-widest mb-1.5 flex items-center gap-1.5">
                ...
                <p class="text-sm font-medium text-gray-800">{{ $ctstatuskatimkerkabag->no_hp ?? '-' }}</p>
            </div>
        </div>
    </div>
        ...
        <div class="p-6 space-y-0 divide-y divide-gray-100">
            <div class="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-gray-100">
                <div class="py-4 px-2">
                    ...
                    <p class="text-sm font-medium text-gray-800">{{ $ctstatuskatimkerkabag->tgl_pengajuan ?? '-' }}</p>
                </div>
                <div class="py-4 px-2 md:pl-6">
                    <p class="text-xs font-medium text-gray-400 uppercase tracking-widest mb-1.5 flex items-center gap-1.5">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                    ...
                    <p class="text-sm font-medium text-gray-800">
                        {{ $ctstatuskatimkerkabag->tgl_mulai ?? '-' }}
                        <span class="text-gray-400 mx-1.5">—</span>
                        {{ $ctstatuskatimkerkabag->tgl_selesai ?? '-' }}
                    </p>
                </div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-gray-100">
                <div class="py-4 px-2">
                    ...
                    <p class="text-sm font-medium text-gray-800">{{ $ctstatuskatimkerkabag->lama_cuti ?? '0' }} hari</p>
                </div>
                <div class="py-4 px-2 md:pl-6">
                    <p class="text-xs font-medium text-gray-400 uppercase tracking-widest mb-1.5 flex items-center gap-1.5">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
                    ...
                    <p class="text-sm font-medium text-gray-800">{{ $ctstatuskatimkerkabag->no_hp_cuti ?? '-' }}</p>
                </div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-gray-100">
                <div class="py-4 px-2">
                    ...
                    <p class="text-sm text-gray-800 leading-relaxed">{{ $ctstatuskatimkerkabag->alasan ?? '-' }}</p>
                </div>
                <div class="py-4 px-2 md:pl-6">
                    <p class="text-xs font-medium text-gray-400 uppercase tracking-widest mb-1.5 flex items-center gap-1.5">
                    ...
                    <p class="text-sm text-gray-800 leading-relaxed">{{ $ctstatuskatimkerkabag->alamat_saat_cuti ?? '-' }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-md overflow-hidden mb-6 border border-gray-100">
        ...
        <div class="p-6">
            ...
            <div class="space-y-0">
                ...
                {{-- Step 1: Ketua Tim Kerja (Disembunyikan jika pemohon adalah Ketua/Kepala Tim Kerja) --}}
                @php
                    $userRole = strtolower(optional($ctstatuskatimkerkabag->user)->role ?? '');
                    $catatanTim = strtolower($catatankatimker ?? '');
                    
                    $isKetuaTim = in_array($userRole, ['kepalatimkerja', 'kepala_tim', 'ketua_tim', 'kepala tim']) 
                                  || str_contains($catatanTim, 'pemohon adalah kepala tim');
                @endphp

                @if(!$isKetuaTim)
                    @php
                        $st1 = strtolower($statuskatimker ?? 'disetujui');
                        if ($st1 === 'ditolak') {
                            ...
                            $s1BadgeClass = 'bg-red-100 text-red-800'; $s1DotClass = 'bg-red-500'; $s1BadgeText = 'Ditolak';
                        } elseif ($st1 === 'perubahan') {
                            $s1Bg = 'bg-indigo-50'; $s1Border = 'border-indigo-400'; $s1Line = 'bg-gray-200';
                            $s1Icon = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#3730A3" stroke-width="2.5"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 .49-4.5"/></svg>';
                            $s1BadgeClass = 'bg-indigo-100 text-indigo-800'; $s1DotClass = 'bg-indigo-500'; $s1BadgeText = 'Perubahan';
                        } else {
                            ...
                            $s1BadgeClass = 'bg-green-100 text-green-800'; $s1DotClass = 'bg-green-500'; $s1BadgeText = 'Disetujui';
                        }
                    @endphp
                    <div class="flex gap-0">
                        ...
                        <div class="flex-1 pl-3 pb-6">
                            <p class="text-sm font-semibold text-gray-800">Ketua Tim Kerja</p>
                            <p class="text-xs text-gray-500 mt-0.5">{{ $ketuaNama ?? '-' }}</p>
                            <p class="text-xs text-gray-400">NIP {{ $ketuaNIP ?? '-' }}</p>
                            @if(!empty($catatankatimker))
                                <div class="mt-2 bg-gray-50 rounded-lg px-3 py-2 text-xs text-gray-600 border border-gray-100">
                                    <span class="font-medium text-gray-500">Catatan:</span> {{ $catatankatimker }}
                                </div>
                            @endif
                            <span class="mt-1.5 inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium {{ $s1BadgeClass }}">
                                <span class="w-1.5 h-1.5 rounded-full {{ $s1DotClass }}"></span>
                                {{ $s1BadgeText }}
                            </span>
                        </div>
                    </div>
                ...
                @php
                    $rawAdmin = $adminStatus ?? $ctstatuskatimkerkabag->ctStatusUserAdmin->status ?? 'disetujui';
                    ...
                    $admName = $adminNama ?? $ctstatuskatimkerkabag->ctStatusUserAdmin->user->name ?? 'Admin Sistem';
                    $admNip  = $adminNIP ?? $ctstatuskatimkerkabag->ctStatusUserAdmin->user->nip ?? '-';
                    $admCatatan = $catatanAdmin ?? $ctstatuskatimkerkabag->ctStatusUserAdmin->catatan ?? null;
                @endphp
                <div class="flex gap-0">
                    <div class="flex flex-col items-center" style="width:40px; flex-shrink:0;">
                    ...
                    <div class="flex-1 pl-3 pb-6">
                        ...
                        <p class="text-xs text-gray-500 mt-0.5">{{ $admName }}</p>
                        <p class="text-xs text-gray-400">NIP {{ $admNip }}</p>
                        @if(!empty($admCatatan))
                            <div class="mt-2 bg-gray-50 rounded-lg px-3 py-2 text-xs text-gray-600 border border-gray-100">
                                <span class="font-medium text-gray-500">Catatan:</span> {{ $admCatatan }}
                            </div>
                        @endif
                ...
                {{-- Step 3: Kepala Bagian Umum --}}
                @php
                    $rawKabag = $statuskabag ?? $ctstatuskatimkerkabag->status ?? null;
                    $st2 = $rawKabag ? strtolower(trim($rawKabag)) : 'disetujui';
                ...
                <div class="flex gap-0">
                    ...
                    <div class="flex-1 pl-3 pb-6">
                        ...
                        <p class="text-xs text-gray-500 mt-0.5">{{ $kepalaBagianUmum->name ?? '-' }}</p>
                        <p class="text-xs text-gray-400">
                            @if(!empty($kepalaBagianUmum->nip)) NIP {{ $kepalaBagianUmum->nip }} @else - @endif
                        </p>
                        @if(isset($ctstatuskatimkerkabag->catatan) && $ctstatuskatimkerkabag->catatan)
                            <div class="mt-2 bg-gray-50 rounded-lg px-3 py-2 text-xs text-gray-600 border border-gray-100">
                                <span class="font-medium text-gray-500">Catatan:</span> {{ $ctstatuskatimkerkabag->catatan }}
                            </div>
                        @endif
            ...
            <div class="flex gap-0">
                ...
                <div class="flex-1 pl-3 pb-0">
                    ...
                    <p class="text-xs text-gray-600 mt-0.5">{{ $kepalaBalai->name ?? '-' }}</p>
                    <p class="text-xs text-gray-400">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-center gap-4 mt-8">
        ...
        @if(!$checkverifikasi)
        <button type="button"
                data-id="{{ $ctstatuskatimkerkabag->id }}"
...
<div id="verifikasiModal" class="fixed inset-0 z-[60] hidden items-center justify-center bg-black/40 backdrop-blur-sm px-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <form id="verifikasiForm" method="POST" action="" class="p-6">
            @csrf
            <input type="hidden" name="" id="dynamic_id_cuti" value="">
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        if (verifikasiForm) {
            verifikasiForm.addEventListener("submit", function (e) {
                ...
                if (currentIsKabag) {
                    dynamicIdInput.name = 'id_ct_status_user_admin';
                } else {
                    dynamicIdInput.name = 'id_ct_status_katimker_kabag';
                }
                dynamicIdInput.value = currentCutiId;

                if (radioDisetujui.checked && radioTtdYa.checked) {
                    verifikasiForm.action = currentIsKabag 
                        ? '{{ route("ctstatuskabagkabal.approve-kabag-with-signature") }}' 
                        : '{{ route("ctstatuskabagkabal.approve-with-signature-kabal") }}';
                } else {
                    verifikasiForm.action = currentIsKabag 
                        ? '{{ route("ctstatuskabagkabal.store-kabag") }}' 
                        : '{{ route("ctstatuskabagkabal.store") }}';
                }
            });
        }
    });
