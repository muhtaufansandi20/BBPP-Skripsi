...
<div class="max-w-6xl mx-auto p-6 pt-0">
        ...
        <div class="p-6 space-y-0 divide-y divide-gray-100">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-gray-100">
                <div class="py-4 px-2">
                    ...
                    <p class="text-sm font-medium text-gray-800">{{ $ctstatususeradmin->name }}</p>
                </div>
                <div class="py-4 px-2 md:pl-6">
                    ...
                    <p class="text-sm font-medium text-gray-800">{{ $ctstatususeradmin->nip }}</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-gray-100">
                <div class="py-4 px-2">
                    ...
                    <p class="text-sm font-medium text-gray-800">{{ $ctstatususeradmin->jabatan }}</p>
                </div>
                <div class="py-4 px-2 md:pl-6">
                    <p class="text-xs font-medium text-gray-400 uppercase tracking-widest mb-1.5 flex items-center gap-1.5">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <p class="text-sm font-medium text-gray-800">{{ $ctstatususeradmin->masa_kerja }}</p>
                </div>
            </div>
            ...
            <div class="py-4 px-2">
                ...
                <p class="text-sm font-medium text-gray-800">{{ $ctstatususeradmin->no_hp }}</p>
            </div>
        ...
        <div class="p-6 space-y-0 divide-y divide-gray-100">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-gray-100">
                <div class="py-4 px-2">
                    ...
                    <p class="text-sm font-medium text-gray-800">{{ $ctstatususeradmin->tgl_pengajuan }}</p>
                </div>
                <div class="py-4 px-2 md:pl-6">
                    <p class="text-xs font-medium text-gray-400 uppercase tracking-widest mb-1.5 flex items-center gap-1.5">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <p class="text-sm font-medium text-gray-800">
                        {{ $ctstatususeradmin->tgl_mulai }}
                        <span class="text-gray-400 mx-1.5">—</span>
                        {{ $ctstatususeradmin->tgl_selesai }}
                    </p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-gray-100">
                <div class="py-4 px-2">
                    ...
                    <p class="text-sm font-medium text-gray-800">{{ $ctstatususeradmin->lama_cuti }} hari</p>
                </div>
                <div class="py-4 px-2 md:pl-6">
                    <p class="text-xs font-medium text-gray-400 uppercase tracking-widest mb-1.5 flex items-center gap-1.5">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <p class="text-sm font-medium text-gray-800">{{ $ctstatususeradmin->no_hp_cuti }}</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-gray-100">
                <div class="py-4 px-2">
                    ...
                    <p class="text-sm text-gray-800 leading-relaxed">{{ $ctstatususeradmin->alasan }}</p>
                </div>
                <div class="py-4 px-2 md:pl-6">
                    <p class="text-xs font-medium text-gray-400 uppercase tracking-widest mb-1.5 flex items-center gap-1.5">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <p class="text-sm text-gray-800 leading-relaxed">{{ $ctstatususeradmin->alamat_saat_cuti }}</p>
                </div>
            </div>
    ...
    <div class="bg-white rounded-xl shadow-md overflow-hidden mb-6 border border-gray-100">
        ...
        <div class="p-6">
            ...
            @php
                $katimkerStatus = strtolower($checkverifikasi->status ?? '');
            @endphp

            <div class="space-y-0">

                {{-- Step 1: Ketua Tim Kerja (Current User) --}}
                <div class="flex gap-0">
                    <div class="flex flex-col items-center" style="width:40px; flex-shrink:0;">
                        @php
                            if ($katimkerStatus === 'disetujui') {
                    ...
                    <div class="flex-1 pl-3 pb-6">
                        <div class="flex items-start justify-between gap-3 flex-wrap">
                            <div>
                                ...
                                {{-- Badge status verifikasi katimker --}}
                                @php
                                    $badgeText  = $katimkerStatus ? ucfirst($katimkerStatus) : 'Anda Belum Memverifikasi';
                                    $badgeClass = match($katimkerStatus) {
                                        'disetujui' => 'bg-green-100 text-green-800',
                                        'ditolak'   => 'bg-red-100 text-red-800',
                                        default     => 'bg-yellow-100 text-yellow-800',
                                    };
                                @endphp
                                <span class="mt-1.5 inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium {{ $badgeClass }}">
                                    <span class="w-1.5 h-1.5 rounded-full
                                        @if($katimkerStatus === 'disetujui') bg-green-500
                                        @elseif($katimkerStatus === 'ditolak') bg-red-500
                ...
                {{-- Step 2: Kepala Bagian Umum (Tampilan Sesuai Gambar) --}}
                <div class="flex gap-0">
                    <div class="flex flex-col items-center" style="width:40px; flex-shrink:0;">
                        <div class="w-8 h-8 rounded-full border border-slate-300 bg-white flex items-center justify-center z-10 shadow-xs">
                            <svg class="w-4 h-4 text-slate-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75">
                                <circle cx="12" cy="12" r="9"/>
                                <line x1="8" y1="12" x2="16" y2="12"/>
                            </svg>
                        </div>
                        <div class="w-0.5 flex-1 bg-gray-200" style="min-height:24px;"></div>
                    </div>

                    <div class="flex-1 pl-3 pb-6">
                        <p class="text-sm font-semibold text-slate-600">Kepala Bagian Umum</p>
                        <p class="text-xs text-slate-500 mt-0.5">
                            {{ $kepalaBagianUmum->name ?? '-' }}
                        </p>
                        <p class="text-xs text-slate-400">
                            NIP {{ $kepalaBagianUmum->nip ?? '-' }}
                        </p>
                        <span class="mt-1.5 inline-flex items-center justify-center px-3 py-0.5 rounded-full text-xs font-semibold bg-slate-100 text-slate-400">
                            -
                        </span>
                    </div>
                </div>

                {{-- Step 3: Kepala Balai (Tampilan Sesuai Gambar) --}}
                <div class="flex gap-0">
                    <div class="flex flex-col items-center" style="width:40px; flex-shrink:0;">
                        <div class="w-8 h-8 rounded-full border border-slate-300 bg-white flex items-center justify-center z-10 shadow-xs">
                            <svg class="w-4 h-4 text-slate-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75">
                    ...
                    <div class="flex-1 pl-3">
                        <p class="text-sm font-semibold text-slate-600">Kepala Balai</p>
                        <p class="text-xs text-slate-500 mt-0.5">
                            {{ $kepalabalai->name ?? '-' }}
                        </p>
                        <p class="text-xs text-slate-400">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-center gap-4 mt-8">
        ...
        <button type="button" onclick="openModal()"
                class="w-full sm:w-auto flex items-center justify-center gap-2 px-8 py-2.5 bg-gradient-to-r from-green-600 to-emerald-500 text-white rounded-xl font-bold hover:shadow-lg hover:scale-105 transition-all">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
{{-- Modal Verifikasi (Card Radio & Desain Konsisten) --}}
<div id="verifikasiModal" class="fixed inset-0 z-[60] hidden items-center justify-center bg-black/40 backdrop-blur-sm px-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        {{-- Form Modal --}}
        <form id="verifikasiForm" method="POST" action="{{ route('ctstatusadminkatimker.store') }}" class="p-6">
            @csrf
            <input type="hidden" name="id_ct_status_user_admin" value="{{ $ctstatususeradmin->id }}">
