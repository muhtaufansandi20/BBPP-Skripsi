    ...
    <div class="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-100 mb-8">
        ...
        <div class="p-5 sm:p-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                        ...
                        <span class="inline-flex items-center gap-1 bg-emerald-100 text-emerald-700 font-semibold px-3 py-1 rounded-full text-xs">
                            ...
                            {{ $pengajuancutitahunan->lama_cuti }} Hari Kerja
                        </span>
                    </div>
                    <div>
                        <p class="text-gray-500 font-medium mb-1">Nomor Telepon</p>
                        <p class="text-gray-800">{{ $pengajuancutitahunan->no_hp_cuti }}</p>
                    </div>
                    <div>
                        <p class="text-gray-500 font-medium mb-1">Alamat Selama Cuti</p>
                        <p class="text-gray-800">{{ $pengajuancutitahunan->alamat_saat_cuti }}</p>
                    </div>
                    <div class="sm:col-span-2">
                        <p class="text-gray-500 font-medium mb-1">Alasan Cuti</p>
                        <p class="text-gray-800">{{ $pengajuancutitahunan->alasan }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-0">
                ...
                <div class="flex gap-0">
                    ...
                    <div class="flex-1 pl-3 {{ $isLast ? 'pb-0' : 'pb-6' }}">
                        ...
                        @if(!$step['blocked'] && isset($step['catatan']) && $step['catatan'] !== '-')
            ...
            <div class="mt-8 flex flex-col-reverse sm:flex-row justify-between items-center gap-4">
                <a href="{{ route('widyaiswarapengajuancutitahunan.index') }}"
                ...
                <div class="flex gap-2 w-full sm:w-auto flex-col sm:flex-row">
                    ...
                    <a href="{{ route('viewPDF', ['id' => $pengajuancutitahunan->id]) }}" target="_blank"
                       class="w-full sm:w-auto flex items-center justify-center gap-2 px-4 py-2.5 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white rounded-lg shadow-sm hover:shadow-md transition-all duration-300">
                    ...
                    @php
                        // Logika bisa membatalkan: status cuti belum dibatalkan, dan admin belum menyetujui
                        $canCancel = true;
                        if (isset($statusAdmin) && $statusAdmin == 'disetujui') $canCancel = false;
                        if (isset($statusKabal) && $statusKabal == 'disetujui') $canCancel = false;
                    @endphp

                    @if(!$pengajuancutitahunan->statusCutiTahunan || $pengajuancutitahunan->statusCutiTahunan->status != 'dibatalkan')
                        @if($canCancel && $statusAdmin != 'disetujui' && $statusKabal != 'disetujui')
                        <form action="{{ route('widyaiswarapengajuancutitahunan.cancel', $pengajuancutitahunan->id) }}" method="POST"
                              onsubmit="return confirm('Apakah Anda yakin ingin membatalkan pengajuan cuti ini? Tindakan ini akan mengembalikan kuota cuti Anda.');" 
                              class="w-full sm:w-auto">
                            @csrf
                            <button type="submit"
                                    class="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-700 hover:to-rose-700 text-white rounded-lg shadow-sm hover:shadow-md transition-all duration-300">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636" />
                                </svg>
                                <span class="text-sm font-medium">Batalkan Pengajuan</span>
                            </button>
                        </form>
                        @endif
                    @endif
