...
class KatimkerPengajuanCutiUmum extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        return view('dashboard.kepalatimkerja.datapengajuancutiumum-katimker', compact(
            'pengajuans', 
            'checkVerifikasi', 
            'user',
            'admin',
    ...
    public function create()
    {
        ...
        return view('dashboard.kepalatimkerja.createpengajuancutiumum-katimker', compact('jenisCuti', 'no_hp_cuti', 'checkUser', 'masakerja'));
    }
   ...
   public function store(Request $request)
    {
        try {
            ...
            $pengajuanId = DB::table('pengajuan_cuti_umums')->insertGetId([
                ...
                'is_katimker' => true, // Set is_katimker to true for Katimker
                'created_at' => now(),
                'updated_at' => now() 
            ]);
            ...
            if ($request->hasFile('lampiran')) {
                ...
                Lampiran::create([
                    'id_pengajuan_cuti' => $pengajuanId, // Fixed: $pengajuanId is already the ID
            ...
            return redirect()->route('katimkerpengajuancutiumum.index')
    ...
    public function show(string $id)
    {
        ...
        return view('dashboard.kepalatimkerja.detailpengajuancutiumum-katimker', compact(
            'pengajuan',
            'ketuaNama', 
            'ketuaNIP', 
            'ketuaJabatan', 
    ...
    public function edit(string $id)
    {
        ...
        return view('dashboard.katimker.editpengajuancutiumum-katimker', compact(
            'pengajuan', 
            'jenisCuti', 
            'lampiran', 
            'no_hp_cuti',
    ...
    public function update(Request $request, string $id)
    {
        ...
        $pengajuanCuti->is_katimker = true; // Ensure is_katimker remains true
        $pengajuanCuti->save();
    ...
    */
    public function cancel($id)
    {
        try {
            ...
            DB::table('status_cuti_umums')->updateOrInsert(
                ...
                [
                    'status' => 'dibatalkan',
                    'catatan' => 'Dibatalkan oleh katimker pada ' . now()->format('d-m-Y H:i:s'),
                    'updated_at' => now(),
                    'created_at' => now()
                ]
            );
            ...
            // Log keberhasilan
            Log::info("Pengajuan cuti katimker dengan ID $id berhasil dibatalkan oleh user ID: $user->id");

            return redirect()->route('katimkerpengajuancutiumum.index')
                ->with('success', '✅ Permohonan cuti Anda telah berhasil dibatalkan.');
        } catch (\Exception $e) {
            // Log the error
            Log::error("Error canceling leave application for katimker: " . $e->getMessage());
    ...
    public function destroy($id)
    {
        try {
            ...
            // 1. Ambil data pengajuan cuti beserta relasinya
            $pengajuanCutiUmum = PengajuanCutiUmum::with([
                'cuStatusUserAdmin.cuStatusAdminKatimker.cuStatusKatimkerKabag.cuStatusKabagKabal'
            ])
            ->where('id', $id)
            ->where('user_id', $user->id)
            ->where('is_katimker', true) // Pastikan filter ini tetap ada sesuai role
            ->first();
            ...
            if ($pengajuanCutiUmum->cuStatusUserAdmin && 
                ...
                return redirect()->route('katimkerpengajuancutiumum.index')
                    ->with('error', "❌ Error: Pengajuan cuti tidak dapat dihapus karena sudah disetujui oleh Admin.");
            }
            ...
            if ($lampiran && $lampiran->file_path) {
                $filePath = $lampiran->file_path; // Format di DB: "lampiran/namafile.pdf"
    
                // Cek file fisik dan hapus
                if (Storage::disk('public')->exists($filePath)) {
                    Storage::disk('public')->delete($filePath);
                    Log::info("✅ File lampiran Katimker dihapus: " . $filePath);
                } else {
            ...
            // 3. Hapus relasi status berjenjang (Manual Cascade)
            if ($pengajuanCutiUmum->cuStatusUserAdmin) {
            ...
            // 4. Hapus status cuti umum (jika ada record terpisah di tabel ini)
            DB::table('status_cuti_umums')->where('id_cuti_umum', $id)->delete();
            
            // 5. Hapus data utama pengajuan
            $pengajuanCutiUmum->delete();
            
            Log::info("Pengajuan cuti katimker ID $id dihapus oleh user: $user->id");
            
            return redirect()->route('katimkerpengajuancutiumum.index')
                ->with('success', "✅ Pengajuan cuti berhasil dihapus.");
    
        } catch (\Exception $e) {
            Log::error("Error deleting Katimker leave: " . $e->getMessage());
    ...
    public function history()
    {
        ...
        return view('dashboard.katimker.pengajuan-cuti.history', compact('pengajuanCuti'));
    }
