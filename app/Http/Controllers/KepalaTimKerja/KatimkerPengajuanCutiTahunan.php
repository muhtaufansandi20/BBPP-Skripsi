...
class KatimkerPengajuanCutiTahunan extends Controller
{ 
    /**
 ...
 */
    ...
    { 
        ...
        // Query pengajuan cuti khusus untuk kepala tim (is_katimker = true)
        ...
        foreach ($pengajuancutitahunan as $cuti) {
            ...
            $cuti->kabagStatus = 'Belum diproses';
            ...
            if ($cuti->ctStatusUserAdmin) {
                ...
                // Untuk ketua tim, lewati approval katimker
                if ($cuti->ctStatusUserAdmin->ctStatusAdminKatimker) {
                    $statusKatimkerKabag = $cuti->ctStatusUserAdmin->ctStatusAdminKatimker->ctStatusKatimkerKabag;
                    if ($statusKatimkerKabag) {
                        $cuti->kabagStatus = $statusKatimkerKabag->status;
                        
                        // Cek status kabal
                        if ($statusKatimkerKabag->ctStatusKabagKabal) {
                            $cuti->kabalStatus = $statusKatimkerKabag->ctStatusKabagKabal->status;
                        }
                    }
                }
            } 
        ...
        return view('dashboard.kepalatimkerja.datapengajuancutitahunan-katimker', compact('pengajuancutitahunan', 'checkVerifikasi', 'hasWorkPeriod', 'hasTeam', 'admin', 'user'));
    }
    ...
    public function create()
    {
        ...
        return view('dashboard.kepalatimkerja.createpengajuancutitahunan-katimker', compact('user', 'checkUser', 'masakerja', 'kuotaCuti'));
    }
    ...
    public function store(Request $request) 
    {
        ...
        // Simpan data pengajuan cuti dengan flag is_katimker = true
        $pengajuanCuti = PengajuanCutiTahunan::create([
            ...
            'is_katimker' => true, // Menandai bahwa ini pengajuan dari ketua tim
        ]);
        ...
        return redirect()->route('katimkerpengajuancutitahunan.index')->with('success', 'Pengajuan Cuti berhasil diajukan.');
    }
    ...
    public function show($id)
        {
            ...
            if ($pengajuancutitahunan->ctStatusUserAdmin) {
                ...
                if ($pengajuancutitahunan->ctStatusUserAdmin->ctStatusAdminKatimker) {
                    $adminKatimker = $pengajuancutitahunan->ctStatusUserAdmin->ctStatusAdminKatimker;
                    
                    if ($adminKatimker->ctStatusKatimkerKabag) {
                        $statusKabag = $adminKatimker->ctStatusKatimkerKabag->status ?? 'Belum diproses';
                        $catatanKabag = $adminKatimker->ctStatusKatimkerKabag->catatan ?? 'Tidak ada catatan';
                        
                        if ($adminKatimker->ctStatusKatimkerKabag->ctStatusKabagKabal) {
                            $statusKabal = $adminKatimker->ctStatusKatimkerKabag->ctStatusKabagKabal->status ?? 'Belum diproses';
                            $catatanKabal = $adminKatimker->ctStatusKatimkerKabag->ctStatusKabagKabal->catatan ?? 'Tidak ada catatan';
                        }
                    }
                }
            }
        
            return view('dashboard.kepalatimkerja.detailpengajuancutitahunan-katimker', compact(
     ...
     * Only allows deletion if the application hasn't been verified by admin yet.
     */
    public function destroy($id)
    {
        try {
            ...
            $pengajuancutitahunan = PengajuanCutiTahunan::where('id', $id)
                ...
                ->where('is_katimker', true) // Make sure it's a team leader application
                ->first();
                
            if (!$pengajuancutitahunan) {
                return redirect()->route('katimkerpengajuancutitahunan.index')
            ...
            if ($pengajuancutitahunan->ctStatusUserAdmin && 
                $pengajuancutitahunan->ctStatusUserAdmin->status != 'Belum diproses') {
                $isVerifiedByAdmin = true;
            }
            
            // If verified by admin, prevent deletion
            if ($isVerifiedByAdmin) {
                return redirect()->route('katimkerpengajuancutitahunan.index')
                    ->with('error', "❌ Error: Pengajuan cuti yang sudah diverifikasi oleh Admin tidak dapat dihapus.");
            }
            
            // Check if the leave application has already been canceled
            if ($pengajuancutitahunan->statusCutiTahunan && 
                $pengajuancutitahunan->statusCutiTahunan->status == 'dibatalkan') {
                return redirect()->route('katimkerpengajuancutitahunan.index')
                    ->with('info', "ℹ️ Pengajuan cuti dengan ID $id sudah dibatalkan sebelumnya.");
            }
            
            // Restore leave quota since we're deleting the application
            $this->restoreLeaveQuota($id);
            
            // Delete related status records first to avoid foreign key constraints
    ...
    public function cancel($id)
    {
        try {
            ...
            if ($pengajuancutitahunan->ctStatusUserAdmin && 
                $pengajuancutitahunan->ctStatusUserAdmin->ctStatusAdminKatimker && 
                $pengajuancutitahunan->ctStatusUserAdmin->ctStatusAdminKatimker->ctStatusKatimkerKabag && 
                $pengajuancutitahunan->ctStatusUserAdmin->ctStatusAdminKatimker->ctStatusKatimkerKabag->ctStatusKabagKabal && 
                $pengajuancutitahunan->ctStatusUserAdmin->ctStatusAdminKatimker->ctStatusKatimkerKabag->ctStatusKabagKabal->status == 'disetujui') {
            ...
            StatusCutiTahunan::updateOrCreate(
                ...
                [
                    'status' => 'dibatalkan',
                    'catatan' => 'Dibatalkan oleh ketua tim kerja pada ' . now()->format('d-m-Y H:i:s')
                ]
            );
