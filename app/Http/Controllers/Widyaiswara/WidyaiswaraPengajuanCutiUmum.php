...
class WidyaiswaraPengajuanCutiUmum extends Controller
{
    /**
    ...
    public function index()
        {
            ...
            return view('dashboard.widyaiswara.datapengajuancutiumum-widyaiswara', compact('pengajuans', 'hasWorkPeriod'));
        }
    ...
    public function create()
    {
        ...
        return view('dashboard.widyaiswara.createpengajuancutiumum-widyaiswara', compact('jenisCuti', 'no_hp_cuti', 'checkUser', 'masakerja'));
    }
...
public function store(Request $request)
    {
        try {
            ...
            return redirect()->route('widyaiswarapengajuancutiumum.index')
    ...
    public function show(string $id)
    {
        ...
        return view('dashboard.widyaiswara.detailpengajuancutiumum-widyaiswara', compact(
            'pengajuan',
            'kepalaBalaiNama', 
            'kepalaBalaiNIP', 
            'kepalaBalaiJabatan',
    ...
    public function destroy(string $id)
    {
        try {
            ...
            ->where('id', $id)
            ->where('user_id', $user->id)
            ->where('is_kabag', true) // Filter khusus Kabag/Widyaiswara sesuai kode asli
            ->first();
            ...
            if ($pengajuan->cuStatusUserAdmin && 
                ...
                return redirect()->route('widyaiswarapengajuancutiumum.index')
                    ->with('error', "❌ Error: Pengajuan cuti tidak dapat dihapus karena sudah disetujui.");
            }
            ...
            if ($lampiran && $lampiran->file_path) {
                ...
                if (Storage::disk('public')->exists($filePath)) {
                    ...
                    Log::info("✅ File lampiran Widyaiswara/Kabag dihapus: " . $filePath);
                } else {
            ...
            Log::error("Error deleting Widyaiswara leave: " . $e->getMessage());
    ...
    public function cancel($id)
    {
        try {
            ...
            DB::table('status_cuti_umums')->updateOrInsert(
                ...
                [
                    'status' => 'dibatalkan',
                    'catatan' => 'Dibatalkan oleh widyaiswara pada ' . now()->format('d-m-Y H:i:s'),
                    'updated_at' => now(),
                    'created_at' => now()
                ]
            );
            ...
            // Log keberhasilan
            Log::info("Pengajuan cuti widyaiswara dengan ID $id berhasil dibatalkan oleh user ID: $user->id");

            return redirect()->route('widyaiswarapengajuancutiumum.index')
                ->with('success', '✅ Permohonan cuti Anda telah berhasil dibatalkan.');
        } catch (\Exception $e) {
            // Log the error
            Log::error("Error canceling leave application for widyaiswara: " . $e->getMessage());
